...
For dynamic quizzes, the React component `app/search-ui/src/Quiz.jsx` fetches a JSON file and handles user interaction:
...
Both the site CSS (`src/style.css`) and the React bundle’s stylesheet (`app/search-ui/src/index.css`) define classes like `.quiz-container`, `.question`, `.choice`, and `.answer` to style quizzes consistently.
