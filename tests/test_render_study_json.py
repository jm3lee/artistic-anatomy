...
def test_render_study_basic():
    index = {"item": {"name": "Foo"}}
    questions = [
        {"q": "Name {{get_desc('item')['title']}}", "c": ["A", "B"], "a": [0, "Because {{get_desc('item')['title']}}"]}
    ]
...
def test_main_outputs_stdout(tmp_path, capsys):
    ...
    index = {"val": {"name": "Bar"}}
    study = [{"q": "{{get_desc('val')['title']}}?", "c": ["X"], "a": [0, "{{get_desc('val')['title']}}"]}]
...
def test_main_writes_file(tmp_path):
    ...
    study_file.write_text(json.dumps([{"q": "{{get_desc('x')['title']}}", "c": ["Y"], "a": [0, ""]}]))
